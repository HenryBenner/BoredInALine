// CRITICAL: Use ONLY Node.js built-ins for immediate startup
// No external dependencies until after health check is ready
import { createServer, IncomingMessage, ServerResponse } from 'http';

const PORT = Number(process.env.PORT || 5000);

// Create raw HTTP server with built-in Node.js (zero dependencies)
const httpServer = createServer((req: IncomingMessage, res: ServerResponse) => {
  // Health check endpoints - respond IMMEDIATELY (but NOT "/" in dev mode - that goes to Expo)
  if (req.url === '/health') {
  ...
  // For all other requests (including "/"), wait for Express to be loaded
  // Express will proxy "/" to Expo in development mode
  if ((global as any).expressApp) {
    (global as any).expressApp(req, res);
  } else {
...
// Start listening IMMEDIATELY with zero dependencies loaded
...
async function initializeApp() {
  // Dynamic imports - load everything in parallel
  const [
    { default: express },
    { default: cors },
    { default: morgan },
  ...
  // Store Express app globally so raw HTTP server can delegate to it
  ...
  // Health check routes (NOT "/" - that goes to Expo proxy in dev mode)
  ...
  // Expo proxy for development
  if (config.nodeEnv !== 'production') {
    const expoProxy = createProxyMiddleware({
      target: `http://localhost:${config.expoPort}`,
      changeOrigin: true,
      ws: false,
    ...
    httpServer.on('upgrade', (req, socket: any, head) => {
      if (!req.url?.startsWith('/socket.io')) {
        expoProxy.upgrade?.(req, socket, head);
      }
    });
    ...
    app.use('/', (req, res, next) => {
      if (req.path.startsWith('/api') || req.path.startsWith('/admin-portal') || 
          ...
          req.path === '/support' || req.path === '/privacy' || req.path === '/terms' || req.path === '/eula' ||
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('send-message', async (data: { barId: string; message?: string; imageUrl?: string; replyToId?: string }) => {
      try {
        ...
        if (user.rows.length === 0) { socket.emit('error', { message: 'User not found' }); return; }
        ...
        if (replyToId) {
          ...
          if (reply.rows.length) { replyToMessage = reply.rows[0].message; replyToUserName = reply.rows[0].name; }
        }
        ...
        io.to(`bar-${barId}`).emit('new-message', {
          id: insert.rows[0].id, barId, userId,
          userName: user.rows[0].name, userImage: user.rows[0].profile_image,
          message: message?.trim() || null, imageUrl: imageUrl || null,
          createdAt: insert.rows[0].created_at, replyToId, replyToMessage, replyToUserName,
        });
